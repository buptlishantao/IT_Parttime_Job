...
def login(i):
    ...
    url='http://m.byr.cn/board/ParttimeJob?p='+str(i);
...
if __name__ == '__main__':
    ...
    curs = conn.cursor()
    curs.execute('''CREATE TABLE if not exists job_haha( 
    ...
    for i in xrange(1,numofpage+1):
        ...
        cursor_id = jobread.find("/article/ParttimeJob/",begin,len(jobread))
        while(cursor_id>0):

            cursor_id_end = jobread.find("\"",cursor_id,len(jobread))
            
            post_id = jobread[cursor_id+21:cursor_id_end]
            ...
            post_url="http://bbs.byr.cn/#!article/ParttimeJob/"+post_id
            ...
            cursor_id = jobread.find("/article/ParttimeJob/",cursor_title_end,len(jobread))
